...
func PkgName(_type reflect.Type) string {
...
//  default path == ""
func ColumnInfos(_type reflect.Type, rootpath string) []ColumnInfo {
	...
	case reflect.Ptr:
		...
		subTags := ColumnInfos(elementType, rootpath) // recursive
		return subTags
	case reflect.Struct:
	...
	for i := 0; i < _type.NumField(); i++ {
		...
		case reflect.Struct:
			if tag, ok := columnTag(field); !ok {
				// struct with no tag
				tags = append(tags, ColumnInfos(field.Type, newbase)...) // recursive
			} else {
				// struct with tag
				tags = append(tags, ColumnInfo{
					Name:    tag.Name,
