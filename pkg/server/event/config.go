...
type RabbitMQNotifierConfig struct {
	...
	MessagePriority        uint8                  `yaml:"message-Publishing-priority"` // queue implementation use - 0 to 9
	MessageCorrelationId   string                 `yaml:"message-correlation-id"`      // application use - correlation identifier
	MessageReplyTo         string                 `yaml:"message-eeply-to"`            // application use - address to to reply to (ex: RPC)
